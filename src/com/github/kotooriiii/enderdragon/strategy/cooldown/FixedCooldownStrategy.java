...
public class FixedCooldownStrategy extends CooldownStrategy {
    ...
    protected void applyCooldown() {
        ...
        ZonedDateTime nextRun= now.withHour(12+4).withMinute(0).withSecond(0).withNano(0);

        if (now.compareTo(nextRun) >= 0)
        {
            nextRun = now.withHour(12+9).withMinute(0).withSecond(0).withNano(0);

            if (now.compareTo(nextRun) >= 0)
            {
                nextRun =  now.plusDays(1).withHour(12+4).withMinute(0).withSecond(0).withNano(0);
            }
        }
